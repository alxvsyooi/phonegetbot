...
ACCOUNT_DEFAULTS: dict[str, Any] = {
    ...
    "mining_enabled": True,
    "mining_time": "01:00",         # якорь (МСК) ТОЛЬКО для ежедневной награды — сбор майнинга на
                                     # своём расписании, см. mining_check_times
    # запускается СТРОГО в эти моменты (МСК), не на интервале — раньше был простой опрос
    # каждые mining_check_interval_minutes от текущего момента, но случайные «Событий» на
    # ферме (см. пример «Взрыв электростанции» — режет питание PSU, может увести в
    # аварийную перегрузку) сделали частый слепой опрос бессмысленным: он просто натыкается
    # на ту же аварию и ничего не решает. Плюс у майнинга есть часовой таймер накопления,
    # который СБРАСЫВАЕТСЯ при каждом выключении фермы (см. farm_maintenance_now — доп.
    # защита min_power_toggle_interval, чтобы не выключать чаще раза в час) — частые
    # проверки с выключением фермы под них буквально теряют накопленное. По умолчанию
    # список покрывает то же самое «раз в 4 часа», что было раньше.
    "mining_check_times": ["01:00", "05:00", "09:00", "13:00", "17:00", "21:00"],
