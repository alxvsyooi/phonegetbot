...
# фиксированный порог для СТАРТА полной прокачки (не настройка, не меняется через бота) —
# полностью прокачать все 7 категорий стоит ощутимых денег, ниже этого даже не начинаем,
# только показываем предупреждение
UPGRADE_MIN_BALANCE_TO_START = 7_000_000
UPGRADE_STEP_DELAY = 1.5             # пауза (сек) между последовательными покупками —
                                       # не долбить игру слишком резко подряд
