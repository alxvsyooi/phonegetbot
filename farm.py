...
class FarmModule:
    ...
    async def _farm_maintenance_now_impl(self) -> str:
        ...
        try:
            ...
            # впустую, если ставить пока нечего (не завершился ремонт и т.п.).
            # occupied<target НЕ участвует в этом условии: target ограничивает
            # только ДОКУПКУ в fill_farm_now() (см. её докстринг — «если уже
            # стоит достаточно (>=target), ничего не докупает, только ставит то,
            # что уже в запасе»), а не установку уже КУПЛЕННЫХ телефонов в
            # реально пустые слоты. Раньше это условие тут тоже проверялось —
            # если общее число слотов фермы выросло (докуплены/разблокированы)
            # за пределы farm_target_phones, occupied уже сам по себе мог
            # оказаться >= target, и уже купленные телефоны (напр. трифолды)
            # так и оставались лежать без дела в «Моих телефонах», а не
            # ставились на реально пустующие слоты (репорт владельца)
            pending_empty_nums: list[int] = []
            if not toggle_cooldown and candidate_by_slot:
